...
noises = repmat(0.0:0.001:0.01, 1, repeats);
...
parfor i = [1:length(noises)]
    ...
    try
        if dataset == 1
            [u, rot, x, labels] = linear_subspace(N, d, n, D, cos, noises(i));
        elseif dataset == 2
            x = yaleX;
            labels = yaleLabels;
        end
...
names = names{1};
save([folder, '/', exp_name, '.mat'], 'err', 'mut', 'dur', 'pred', 'cs', 'rep', 'names', 'noises')
